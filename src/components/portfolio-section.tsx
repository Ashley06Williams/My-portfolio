...
export default function WorkSection() {
  ...
  return (
    <section id="portfolio" className=" scroll-mt-32 section-spacing bg-white rounded-[60px] p-10">
      ...
      <h3 className="text-[#1D202F] text-2xl font-light">Featured Work</h3>
      <h1 className="font-semibold text-6xl text-[#1D202F] mt-4 mb-14">
        <span className="text-red">Port</span>folio
      </h1>
      </div>

<div className="w-full">
      {/* Mobile stacked list (visible on small screens only) */}
      <div className="block md:hidden">
        <div className="flex flex-col">
          {workItem.map((item) => (
            <div key={item.id} className="mb-6">
              {item.workLink ? (
                <Link target="_blank" href={item.workLink} className="group block">
                  <div className="group bg-blueGrey-light shadow-md rounded-3xl w-full flex flex-col">
                    <div className="flex items-center justify-center mb-4 px-2 pt-2">
                      <img src={item.imgSrc} alt={item.title} className="rounded-3xl w-full object-cover" />
                    </div>
      ...
      {/* Carousel (visible on md+ screens) */}
